...
*** init-ai.el
AI feature-module: gptel + gptel-agent + karthink/gptel-preset-collection. Default backend is Zhipu GLM (coding-plan endpoint from the previous ai-agent config). Dedicated chat buffers use Org and gptel's margin response highlight. Inline completion is minuet on the same GLM endpoint.
