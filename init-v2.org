...
*** init-ai.el
AI feature-module entry point: package dependency declarations and keybindings. The omy-ai implementation (sessions, agent wiring, workflow commands, backends, chat UI) lives in the standalone emacs-agent repo at ~/project/emacs-agent, referenced via load-path.
