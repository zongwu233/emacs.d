...
windows 环境下安装 nerd-icons-fonts,执行 M-x nerd-icons-install-fonts,需要使用代理，
下载之后，手动安装fonts文件。重启 emcas。
在 macOS 中使用了系统改键，调整了CapsLock按键效果为Ctrl。
在 Windows 环境中，使用了 AutoHotKey 软件达成相同效果。
*** early-init.el
在 init.el 配置之前加载
