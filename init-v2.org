...
** 模块化配置
*** intro
采用 literate programming 组织 emacs 配置文件。这是目前我发现的最快速的上手方式。
PS：Org-babel在一次 tangle 过程中，对于同一个目标文件，先聚合然后顺序写入。

base on https://github.com/condy0919/.emacs.d
当前使用 Emacs 版本 29.4

本配置依赖 nerd-icons, 请确保 M-x nerd-icons-install-fonts 安装对应的字体以显示图标。
windows 环境下安装 nerd-icons-fonts,执行 M-x nerd-icons-install-fonts,需要使用代理，下载之后，手动安装fonts文件。重启 emcas。

在 macOS 中使用了系统改键，调整了CapsLock按键效果为Ctrl。在 Windows 环境中，使用了 AutoHotKey 软件达成相同效果。
*** early-init.el
在 init.el 配置之前加载
...
#+END_SRC
*** init-leader-key.el
global leader key define and keybindgs
#+BEGIN_SRC emacs-lisp  :tangle ~/.emacs.d/lisp/init-leader-key.el
  ;; -*- coding: utf-8; lexical-binding: t; -*-
...
#+end_src
*** init-haskell.el
Haskell 配置：=haskell-mode= + 内置 =eglot= 接 =haskell-language-server=。

依赖（系统层，通过 ghcup 安装）：
- =ghc= / =cabal= / =haskell-language-server-wrapper= 需在 PATH
- PATH 注入由 =exec-path-from-shell= 负责（init-basic 配置）

#+begin_src emacs-lisp :tangle ~/.emacs.d/lisp/init-haskell.el
;; -*- coding: utf-8; lexical-binding: t; -*-
...
*** init-md.el
markdown 文件配置相关
#+begin_src emacs-lisp :tangle ~/.emacs.d/lisp/init-md.el
...
*** init-ai.el
AI feature-module entry point: package dependency declarations and keybindings. The omy-ai implementation (sessions, agent wiring, workflow commands, backends, chat UI) lives in the standalone emacs-agent repo, cloned on first start from the private GitHub remote (git@github.com:zongwu233/emacs-agent.git) into ~/.emacs.d/site-lisp/emacs-agent and referenced via load-path. On a machine without git or GitHub SSH access initialization still completes; only the AI menu (SPC a) and minuet stay disabled, with a startup warning showing the fix.
