...
(defun my/gptel-restore-session-buffer ()
  "Restore `gptel-mode' when visiting a saved gptel session."
  (when (and buffer-file-name
             (file-in-directory-p buffer-file-name my/gptel-session-directory)
             (derived-mode-p 'org-mode 'markdown-mode 'text-mode)
             (not (bound-and-true-p gptel-mode)))
    (gptel-mode 1)))

(add-hook 'find-file-hook #'my/gptel-restore-session-buffer)
